(function() {
    ...
    const fixtureMap = [
        {
            match: (url) => url.pathname === '/backend/v1/topology1/topology_multilanguage',
            fixture: '../docs/fixtures-flow-multilang.json'
        },
        {
            match: (url, method) => method === 'GET' && url.pathname === `/backend/flow/${solutionId}`,
            fixture: '../docs/fixtures-flow-35.json'
        },
        {
            match: (url, method) => method === 'GET' && url.pathname === `/backend/flow/usertask/${solutionId}`,
            fixture: '../docs/fixtures-flow-usertask-35.json'
        },
        {
            match: (url) => url.pathname === '/backend/v1/flow/device_methods',
            fixture: '../docs/fixtures-flow-device-methods.json'
        },
        {
            match: (url) => url.pathname === `/backend/v1/solutions/${solutionId}/topologies`,
            fixture: '../docs/fixtures-flow-solution-topologies.json'
        },
        {
            match: (url) => url.pathname === '/backend/v1/companies/1/device-types',
            fixture: '../docs/fixtures-flow-device-types.json'
        },
        {
            match: (url) => url.pathname === '/backend/v1/flow/topology/common_modbus_device_methods',
            fixture: '../docs/fixtures-flow-common-modbus-device-methods.json'
        },
        {
    ...
    function resolveFixture(url, method) {
        ...
        if (url.pathname === '/backend/v1/refresh') return { type: 'json', value: genericOk };
        if (url.pathname === '/backend/program') return { type: 'json', value: genericOk };
        if (url.pathname === '/backend/flow/section') return { type: 'json', value: genericOk };
        if (url.pathname === '/backend/step') return { type: 'json', value: genericOk };
        if (url.pathname.startsWith('/backend/v1/topologies/') && url.pathname.includes('/devices/') && url.pathname.endsWith('/setting-props')) {
            return { type: 'json', value: tableOk };
        }
        if (url.pathname.startsWith('/backend/flow/') && normalizedMethod !== 'GET') return { type: 'json', value: genericOk };
    ...
    window.WebSocket = function(url, protocols) {
        if (typeof url === 'string' && (url.includes('/mqtt') || url.includes('/middle/ws/'))) {
            this.url = url;
            this.protocol = '';
            this.readyState = 0;
    ...
    window.localStorage.setItem('versionControl', JSON.stringify({ version_name: 'platform' }));
